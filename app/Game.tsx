...
type Fighter = { x:number; y:number; vx:number; vy:number; w:number; h:number; color:string; accent:string; face:number; hp:number; maxHp:number; mana:number; maxMana:number; jumps:number; grounded:boolean; attack:number; cooldown:number; dash:number; dashBurst:number; fireCooldown:number; invuln:number; dead:number; score:number; name:string; defending:boolean; guardCancel:number };
...
type ManaPickup = { x:number;y:number;phase:number;collected:boolean };
...
function fighter(x:number,color:string,accent:string,name:string):Fighter { return {x,y:670,vx:0,vy:0,w:62,h:82,color,accent,face:1,hp:100,maxHp:100,mana:5,maxMana:5,jumps:2,grounded:false,attack:0,cooldown:0,dash:0,dashBurst:0,fireCooldown:0,invuln:0,dead:0,score:0,name,defending:false,guardCancel:0}; }
...
export default function Game(){
  ...
  const [hud,setHud]=useState<{p1:number;p2:number;p1Mana:number;p2Mana:number;remaining:number;progress:number}>({p1:100,p2:100,p1Mana:5,p2Mana:5,remaining:enemySeeds.length,progress:0});
  ...
  const reset=useCallback(()=>{
    game.current={players:[fighter(180,"#b8332e","#e6c24c","BRAMBLE"),fighter(270,"#297388","#d8d0b6","THIMBLE")],enemies:enemySeeds.map(([x,y,k],i)=>({...fighter(x,k==="badger"?"#5c4a3d":k==="hare"?"#9a7653":"#4b3a64",k==="badger"?"#d55337":k==="hare"?"#6c9d4c":"#be4b75",k.toUpperCase()),y,w:k==="bat"?58:64,h:k==="bat"?55:76,kind:k,home:x,active:false,defeated:false,ai:i*.7})),pickups:manaSeeds.map(([x,y],i)=>({x,y,phase:i*1.73,collected:false})),particles:[],projectiles:[],camera:0,last:performance.now(),gateOpen:false,shake:0};
    stateRef.current="play";setScreen("play");setHud({p1:100,p2:100,p1Mana:5,p2Mana:5,remaining:enemySeeds.length,progress:0});startMusic();
  },[startMusic]);
  ...
  useEffect(()=>{
    ...
    function hit(target:Fighter,from:Fighter,power:number,sourceX?:number){if(target.invuln>0||target.dead>0)return;const attackX=sourceX??from.x+from.w/2;if(target.defending&&(attackX-(target.x+target.w/2))*target.face>0){target.invuln=.12;target.vx-=target.face*1.5;burst(target.x+target.w/2+target.face*34,target.y+35,"#f3cf67",6);playSfx("hit",.3,.09);return;}target.hp-=power;const g=game.current;if(g?.players.includes(target))g.shake=Math.max(g.shake,Math.min(6,2.7+power*.14));target.vx=from.face*(8+power*.22);target.vy=-5-power*.12;target.invuln=.36;burst(target.x+target.w/2,target.y+target.h/2,target.accent,9);playSfx("take_damage",.5,.08);if(target.hp<=0){target.dead=1.6;target.defending=false;target.vy=-13;target.vx=from.face*13;from.score++;burst(target.x+target.w/2,target.y+target.h/2,"#f5d66d",20);playSfx("death",.72,.035);}}
    function updatePlayer(p:Fighter,i:number,dt:number){if(p.dead>0){p.defending=false;p.dead-=dt;p.vy+=34*dt;p.x+=p.vx*60*dt;p.y+=p.vy*60*dt;if(p.dead<=0){p.hp=100;p.x=Math.max(120,(game.current?.camera||0)+180+i*90);p.y=560;p.vx=p.vy=0;p.invuln=2;p.guardCancel=0;}return;}
      ...
      if(p.attack>0.08&&p.attack<.2){const blade=swordBounds(p);let struckEnemy=false;for(const e of game.current?.enemies||[])if(!e.defeated&&e.invuln<=0&&overlaps(blade,e)){hit(e,p,18);struckEnemy=true;}for(const other of game.current?.players||[])if(other!==p&&other.dead<=0&&overlaps(blade,playerBounds(other)))hit(other,p,18);if(struckEnemy){p.vx=-p.face*4.4;p.vy=Math.min(p.vy,-1.4);burst(p.x+p.w/2+p.face*70,p.y+28,"#fff1a8",5);}}
    }
    ...
    function loop(t:number){
      ...
      if(g){
        ...
        if(stateRef.current==="play"){
          ...
          for(const pickup of g.pickups){if(pickup.collected)continue;const bottle={x:pickup.x-20,y:manaY(pickup)-32,w:40,h:54};for(const player of g.players){if(player.dead<=0&&player.mana<player.maxMana&&overlaps(bottle,playerBounds(player))){pickup.collected=true;player.mana=Math.min(player.maxMana,player.mana+3);burst(pickup.x,manaY(pickup),"#5cecff",16);break;}}}
          for(const e of g.enemies)updateEnemy(e,dt);
          for(const f of g.projectiles){f.x+=f.vx;f.life-=dt;const fireball={x:f.x-16,y:f.y-16,w:32,h:32};for(const e of g.enemies){if(!e.defeated&&f.life>0&&overlaps(fireball,e)){hit(e,f.owner,20,f.x);burst(f.x,f.y,f.color,14);playSfx("fireball_hit",.58,.07);f.life=0;}}for(const player of g.players){if(player!==f.owner&&player.dead<=0&&f.life>0&&overlaps(fireball,playerBounds(player))){hit(player,f.owner,20,f.x);burst(f.x,f.y,f.color,14);playSfx("fireball_hit",.58,.07);f.life=0;}}}
          ...
          if(Math.floor(t/120)%4===0)setHud({p1:Math.max(0,g.players[0].hp),p2:Math.max(0,g.players[1].hp),p1Mana:g.players[0].mana,p2Mana:g.players[1].mana,remaining:alive,progress:clamp(lead/(WORLD-240)*100,0,100)});
